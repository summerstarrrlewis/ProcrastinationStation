...
# =================== GLOBAL VARIABLES ===================
# These variables track UI state, task data, and timer info
finished_tasks = []  # Stores all completed task names for display on the dashboard
focus_widgets = []  # Holds widget references for focus screen, so they can be destroyed/cleared easily
focus_flash_canvas_items = []  # Canvas items for encouragement flash
user_minutes = 0  # Task duration in minutes, as entered by the user
current_task_name = ""  # Name of the task currently being worked on
current_timer = None  # Stores reference to the active timer event (for cancelling if needed)
checkin_canvas_items = []  # Canvas items for check-in overlays
checkin_widgets = []  # Widgets (buttons) for check-in screens
last_timer_state = {"seconds_left": None, "checkin_points": None}  # Used to restore timer state (e.g., after break)

WINDOW_WIDTH, WINDOW_HEIGHT = 311, 452  # GUI window size
...
def clear_widget_list(lst):
    """Destroy or delete all widgets/canvas items in the given list, then clear the list."""
    for w in lst:
...
def clear_all_for_screen():
    """Clears all widget and canvas item lists for a fresh screen."""
...
# =================== INTRO SCREEN SETUP ===================
# Set up the intro Canvas (startup screen)
canvas = tk.Canvas(root, width=WINDOW_WIDTH, height=WINDOW_HEIGHT, highlightthickness=0)
canvas.pack()

# Load and display the intro background and chat bubble images
...
# =================== DASHBOARD ASSETS (Images, Canvas) ===================
...
# =================== TIMER & CHECK-IN LOGIC ===================
def show_timer_screen(minutes, checkin_count): #explainer vid
    """
    Starts the timer screen for a task with given minutes and check-in count.
    Sets up background and figures out when check-ins should occur.
    """
    ...
    dashboard_canvas.timer_img = timer_img  # Reference to avoid image garbage collection

    # Calculate total seconds for timer and check-in intervals
...
def run_timer(seconds_left, checkin_points):
    """
    Main timer loop: updates the timer display, handles check-in popups,
    and triggers celebration when finished.
    """
    dashboard_canvas.delete("timer_text")
    clear_widget_list(checkin_canvas_items)
    clear_widget_list(checkin_widgets)
    ...
    # Add a "Finished Task" button beneath the timer if not at a check-in and time remains
    ...
    # Show check-in overlay if needed at this second
    ...
    # Continue timer loop if time remains, otherwise show "Time's up!"
    if seconds_left > 0:
        global current_timer
        current_timer = root.after(1000, lambda: run_timer(seconds_left - 1, checkin_points))
    else: # Show "Time's up!" and finished button
        dashboard_canvas.delete("timer_text")
...
def show_canvas_checkin(seconds_left, checkin_points):
    """
    Display a check-in overlay: asks if user is still on track.
    If "You know it!" is chosen, continues timer; else, shows distraction screen.
    """
    dashboard_canvas.delete("timer_text")
    ...
    def on_keep_going():
        """User says they're still on track: resume timer."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        if seconds_left == 0:
    ...
    def on_distracted():
        """User says they're distracted: offer break/restart options."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        show_distracted_screen()

    # Two buttons: "You know it!" and "Not really..."
...
def show_distracted_screen():
    """
    If the user is distracted, this screen offers options:
    Keep going, Take a break, Restart timer, or Restart task.
    """
    dashboard_canvas.delete("timer_text")
    clear_widget_list(checkin_canvas_items)
    clear_widget_list(checkin_widgets)
    ...
    def keep_going():
        """Resume timer where left off."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        run_timer(last_timer_state["seconds_left"], last_timer_state["checkin_points"])

    def take_a_break():
        """Show break timer screen."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        show_break_timer()

    def restart_timer():
        """Restart the timer entry screen."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        show_time_entry_screen()

    def restart_task():
        """Return to dashboard to start a new task."""
        clear_widget_list(checkin_canvas_items)
        clear_widget_list(checkin_widgets)
        show_dashboard()

    # Four buttons: each calls a different handler above
    btns = [
...
def show_break_timer(): #explainer vid
    """
    Shows a 5-minute break timer, then offers to return to task.
    """
    clear_widget_list(checkin_canvas_items)
    clear_widget_list(checkin_widgets)
    dashboard_canvas.delete("timer_text")
    ...
    def update_break_timer(secs_left):
        """Countdown and update display each second."""
        dashboard_canvas.delete("timer_text")
...
def show_celebration_screen():
    """
    Celebration screen for finishing the task.
    Animates background, flips cat image, and adds task to finished list.
    """
    global finished_tasks, current_task_name, current_timer
    # Add completed task to the finished tasks list (if not already there)
    ...
    if current_timer is not None:
        ...
        current_timer = None
    clear_all_for_screen()
    # Remove all items except for main background
    for item in dashboard_canvas.find_all():
        if dashboard_canvas.type(item) == "image":
            ...
            dashboard_canvas.delete(item)
    # Animate rainbow stripes background
    ...
    animate_rainbow()
    # Add celebration cat and text overlays (plus animation flip)
    ...
    def flip():
        """Animates the cat by flipping the image back and forth."""
    ...
    flip()
    def next_task():
        """Return to dashboard, showing the updated finished tasks list."""
        clear_widget_list(checkin_widgets)
        for item in rects:
            dashboard_canvas.delete(item)
...
def show_focus_flash(png_path, message, minutes, checkin_count):
    """
    Shows a brief encouragement/feedback graphic before timer starts.
    Then transitions to the timer screen.
    """
    clear_all_for_screen()
...
def show_focus_screen(): #explainer vid
    """
    Lets user choose their focus level for this session.
    This determines how many check-ins will occur.
    """
    ...
    nudgespeech3_img = tk.PhotoImage(file="NUDGESPEECH_3.png") #explainer vid
    ...
    # Layout buttons for the four focus levels
    ...
    # Each button sets the number of check-ins based on user focus
...
def show_time_entry_screen(): #explainer vid comment
    """
    Prompts the user to enter how many minutes the task should take.
    On enter, saves value and moves to focus selection.
    """
    clear_widget_list(checkin_canvas_items)
    clear_widget_list(checkin_widgets)
    dashboard_canvas.delete("timer_text")

    nudgespeech2_img = tk.PhotoImage(file="NUDGESPEECH_2.png") #explainer vid comment
    ...
    BORDER_COLOR = "#c97a11"
    # Draw input box for minutes entry
    ...
    time_entry = tk.Entry(root, font=("Arial", 12, "bold"), justify="center", fg="#888888", bg="#fffbe3", #explainer vid
    ...
    # Helper functions for input field UI/validation
...
def show_dashboard(): #explainer vid comment
    """
    Main dashboard: lets user enter project name, new tasks, and see finished tasks list.
    Handles entry and state for starting a new task session.
    """
    canvas.pack_forget() #explainer vid comment
    dashboard_canvas.pack()
    dashboard_canvas.create_image(0, 0, anchor=tk.NW, image=main_bg)

    # Project name at top + explainer vid comment
    ...
    # Finished tasks label and columns + explainer vid comment
    ...
    # Show initial helper images and task entry box
    ...
    def on_task_entry_return(event): #explainer vid comment
        """Triggered when user presses Enter on new task box."""
        global current_task_name
        task = task_entry.get().strip()
        if not task or task == "Type your task here...":
            return
        current_task_name = task
        # Transition to time entry screen for this task
        ...
        dashboard_canvas.create_window(MIN_BOX_X, MIN_BOX_Y, window=time_entry, width=MIN_BOX_WIDTH,
                                      height=MIN_BOX_HEIGHT, tags="time_entry")
        time_entry.focus()
        # Helper functions for minutes input events
        def on_time_entry_click(event):
            if event.widget.get() == "Minutes":
                event.widget.delete(0, tk.END)
                event.widget.config(fg="#333333")
    ...
    # Setup and bind the task entry box + explainer vid comment
...
# =================== INTRO BUTTON EVENTS ===================
def on_press(e):
    """Visual feedback for intro button press (shadow, color)."""
    canvas.itemconfig(button, fill="#FFD54F")
    canvas.coords(shadow, btn_x + 1, btn_y + 1, btn_x + btn_w + 1, btn_y + btn_h + 1)
def on_release(e):
    """On release, restores button color and transitions to dashboard."""
